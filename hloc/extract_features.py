...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, conf, paths=None, mask_dir=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_dir is not None:
            mask_name = str(self.mask_dir / name) + ".png"
        ...
        if self.conf.resize_max and (
            ...
            # if mask is not None:
            #     mask = resize_image(mask, size_new, self.conf.interpolation)
...
def main(
    ...
    for idx, data in enumerate(tqdm(loader)):
        ...
        #     mask = data["mask"]
        #     # B = 1, C = 1
        #     print(data["image"].shape, mask.shape)
        #     print(pred["keypoints"][:, 1].max(), pred["keypoints"][:, 0].max())
        #     valid_keypoint = mask[
        #         0,
        #         0,
        ...
        if mask_dir is not None:
            ...
            # print(data["image"].shape, mask.shape)
            # print(pred["keypoints"][:, 1].max(), pred["keypoints"][:, 0].max())
            keypoint_mask = np.logical_and(pred["keypoints"][:, 1] < mask.shape[2], pred["keypoints"][:, 0] < mask.shape[3])
            valid_keypoint = mask[
                0,
                0,
                pred["keypoints"][keypoint_mask, 1].astype("int"),
                pred["keypoints"][keypoint_mask, 0].astype("int"),
            ]
            ...
            # features like LoFTR do not have scores
            if "scores" in pred:
